	...
	function removeEvents(filter) {
		if (!filter) { 
			...
			for (var j=0; j<sources.length; j++) {
				if ($.isArray(sources[j].events)) {
					sources[j].events = $.grep(sources[j].events, filter, true);
				}
			}
		}
		reportEvents(cache);
		...
		if (c == "'") {
			...
			for (i2=i+1; i2<len; i2++) {
				if (format.charAt(i2) == ']') {
					...
					var subres2 = formatDate(date, subformat, options);
					if (subres2 != formatDate(otherDate, subformat, options)) {
						res += subres2;
					}
					i = i2;
					break;
...
function DayEventRenderer() {
	...
	function setVerticals(segments, doRowHeights) {
		...
		for (var j=0; j<rowContentElements.length; j++) {
			rowContentTops.push(
				rowContentElements[j].position().top
			);
		}
	...
	function calculateVerticals(segments) {
		...
		for (var rowI=0; rowI<rowCnt; rowI++) {
			...
			for (var segmentI=0; segmentI<segmentRow.length; segmentI++) {
				...
				for (var colJ=segment.leftCol; colJ<=segment.rightCol; colJ++) {
					colHeights[colJ] = segment.top + segment.outerHeight;
				}
			}
